...
(defproject users-msvc "0.0.1-SNAPSHOT"
  ...
  :profiles {:dev {:aliases {"run-dev" ["trampoline" "run" "-m" "users-msvc.server/run-dev"]}
                   :dependencies [[io.pedestal/pedestal.service-tools "0.6.0"]]}
             :uberjar {:aot [users-msvc.server]}}
  :main ^{:skip-aot true} users-msvc.server)
